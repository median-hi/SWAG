package at.jku.dke.swag.analysis_graphs.operations.operation_types.dice_ops;


import at.jku.dke.swag.AppConstants;
import at.jku.dke.swag.analysis_graphs.AnalysisSituation;
import at.jku.dke.swag.analysis_graphs.basic_elements.Pair;
import at.jku.dke.swag.analysis_graphs.operations.Operation;
import at.jku.dke.swag.analysis_graphs.utils.Utils;
import at.jku.dke.swag.md_elements.Level;
import at.jku.dke.swag.md_elements.MDGraph;
import at.jku.dke.swag.md_elements.init.MDGraphInit;
import org.junit.jupiter.api.*;

import java.util.List;
import java.util.Set;

public class ChangeDiceLevelToTest {

    MDGraph mdGraph;
    AnalysisSituation source;
    AnalysisSituation target;

    AnalysisSituation opTarget;
    Set<Operation> ops;

    @BeforeEach
    void initMDGraph() {
        this.mdGraph = MDGraphInit.initMDGraph();
    }

    @Nested
    @DisplayName("When actual dice level is constant and new value is different")
    class ActualLevelIsConstant {

        @Test
        @DisplayName("When actual dice level is constant and new value is different")
        void added1() {
            source = createSource();
            target = createTarget();
            ops = initOperations();
            AnalysisSituation sourceCopy = source.copy();
            sourceCopy.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.EU);
            opTarget = Utils.evaluateAndFire(sourceCopy, ops);
            Assertions.assertFalse(Utils.evaluate(source, ops).isEmpty());
            opTarget.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.EU);
            Assertions.assertEquals(opTarget, target);
        }

        public Set<Operation> initOperations() {
            Operation op3 = new Operation(ChangeDiceLevelTo.getInstance(),
                    List.of(AppConstants.DESTINATION_DIM, AppConstants.CONTINENT));
            return Set.of(op3);
        }

        public AnalysisSituation createSource() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, AppConstants.GEO);
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.AUSTRIA);
            return as;
        }

        public AnalysisSituation createTarget() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, AppConstants.CONTINENT);
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.EU);
            return as;
        }
    }

    @Nested
    @DisplayName("When actual dice level is constant and new value is same")
    class ActualLevelIsConstantAndNewValueIsSame {

        @Test
        @DisplayName("When actual dice level is constant and new value is same")
        void added1() {
            source = createSource();
            target = createSource();
            ops = initOperations();
            opTarget = Utils.evaluateAndFire(source, ops);
            Assertions.assertTrue(Utils.evaluate(source, ops).isEmpty());
            Assertions.assertEquals(opTarget, target);
        }

        public Set<Operation> initOperations() {
            Operation op3 = new Operation(ChangeDiceLevelTo.getInstance(),
                    List.of(AppConstants.DESTINATION_DIM, AppConstants.GEO));
            return Set.of(op3);
        }

        public AnalysisSituation createSource() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, AppConstants.GEO);
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.AUSTRIA);
            return as;
        }
    }

    @Nested
    @DisplayName("When actual dice level is a bound value of a parameter")
    class ActualLevelIsBound {

        @Test
        @DisplayName("When actual dice level is a bound value of a parameter")
        void added1() {
            source = createSource();
            target = createTarget();
            ops = initOperations();
            AnalysisSituation sourceCopy = source.copy();
            sourceCopy.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.EU);
            opTarget = Utils.evaluateAndFire(sourceCopy, ops);
            Assertions.assertFalse(Utils.evaluate(source, ops).isEmpty());
            Assertions.assertEquals(opTarget, target);
        }

        public Set<Operation> initOperations() {
            Operation op3 = new Operation(ChangeDiceLevelTo.getInstance(),
                    List.of(AppConstants.DESTINATION_DIM, AppConstants.CONTINENT));
            return Set.of(op3);
        }

        public AnalysisSituation createSource() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, new Pair(AppConstants.GRAN_PARAM, AppConstants.GEO));
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.AUSTRIA);
            return as;
        }

        public AnalysisSituation createTarget() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, new Pair(AppConstants.GRAN_PARAM, AppConstants.CONTINENT));
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.EU);
            return as;
        }
    }

    @Nested
    @DisplayName("When actual dice level is an unbound value of a parameter")
    class ActualLevelIsUnBound {

        @Test
        @DisplayName("When actual dice level is an unbound value of a parameter")
        void added1() {
            source = createSource();
            target = createTarget();
            ops = initOperations();
            opTarget = Utils.evaluateAndFire(source, ops);
            Assertions.assertFalse(Utils.evaluate(source, ops).isEmpty());
            Assertions.assertEquals(opTarget, target);
        }

        public Set<Operation> initOperations() {
            Operation op3 = new Operation(ChangeDiceLevelTo.getInstance(),
                    List.of(AppConstants.DESTINATION_DIM, AppConstants.GEO));
            return Set.of(op3);
        }

        public AnalysisSituation createSource() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, new Pair(AppConstants.GRAN_PARAM, Level.unknown()));
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.AUSTRIA);
            return as;
        }

        public AnalysisSituation createTarget() {
            AnalysisSituation as = new AnalysisSituation(mdGraph);
            as.setDiceLevel(AppConstants.DESTINATION_DIM, new Pair(AppConstants.GRAN_PARAM, AppConstants.GEO));
            as.setDiceNode(AppConstants.DESTINATION_DIM, AppConstants.AUSTRIA);
            return as;
        }
    }
}
